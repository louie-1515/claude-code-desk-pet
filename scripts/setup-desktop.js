...
const lnkPath = path.join(desktopDir, "Claude 桌宠启动器.lnk");
...
ico = r'${iconPath}'
cfg_path = r'${path.join(projectRoot, 'assets', 'pet.json')}'
with open(cfg_path, 'r', encoding='utf-8') as f:
    cfg = json.load(f)
cw, ch = cfg['cell']['width'], cfg['cell']['height']
row = cfg['animations']['idle']['row']
img = Image.open(r'${path.join(projectRoot, 'assets', 'spritesheet.png')}')
...
// Step 3: Create shortcut via PowerShell (at temp ASCII path, then move)
if (existsSync(lnkPath)) {
  console.log("Desktop shortcut already exists, skipping.");
  process.exit(0);
}
